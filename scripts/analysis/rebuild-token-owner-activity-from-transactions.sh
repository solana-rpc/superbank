...
usage() {
  cat <<'EOF'
Usage: scripts/analysis/rebuild-token-owner-activity-from-transactions.sh <start_epoch> <end_epoch>

Rebuilds token owner activity rows from transactions_local in epoch windows, where:
...
Environment:
  ...
  TOKEN_OWNER_ACTIVITY_TABLE      Destination table (default: default.token_owner_activity)
EOF
}
...
build_query() {
  ...
  cat <<SQL
INSERT INTO ${TOKEN_OWNER_ACTIVITY_TABLE}
WITH
    [
...
run_epoch_on_host() {
  ...
  log "[epoch=${epoch} shard=${shard_num} host=${host}] start target=${TOKEN_OWNER_ACTIVITY_TABLE}"
...
TOKEN_OWNER_ACTIVITY_TABLE="${TOKEN_OWNER_ACTIVITY_TABLE:-default.token_owner_activity}"
