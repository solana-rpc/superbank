...
: "${GET_BLOCK_RESPONSE_CACHE_MAX_BYTES:=0}"
: "${RPC_RESPONSE_GZIP_ENABLED:=false}"
...
: "${CLICKHOUSE_REPLICA_HEALTH_CHECK_INTERVAL_MS:=10000}"
...
export GET_BLOCK_RESPONSE_CACHE_MAX_BYTES
export RPC_RESPONSE_GZIP_ENABLED
...
export CLICKHOUSE_REPLICA_HEALTH_CHECK_INTERVAL_MS
...
./target/release/superbank-rpc \
  ...
  --get-block-response-cache-max-bytes "${GET_BLOCK_RESPONSE_CACHE_MAX_BYTES}" \
