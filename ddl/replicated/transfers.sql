...
-- Transfer ledger tables with replicated shard-local storage used by getTransfersByAddress.
CREATE TABLE IF NOT EXISTS default.transfers_local ON CLUSTER '{cluster}'
(
...
CREATE MATERIALIZED VIEW IF NOT EXISTS default.transfers ON CLUSTER '{cluster}'
(
    signature FixedString(64),
    slot UInt64,
    slot_idx UInt32,
...
ENGINE = Distributed('{cluster}', 'default', 'transfers_local', cityHash64(signature))
AS
SELECT
    signature,
    slot,
